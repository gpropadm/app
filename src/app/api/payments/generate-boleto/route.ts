...
export async function POST(request: NextRequest) {
  try {
    const { companyId, amount, dueDate, customer, description } = await request.json()
    
    console.log('💳 Generating boleto for company:', { 
      companyId, 
      amount, 
      dueDate, 
    ...
    if (!credentials) {
      return NextResponse.json({ 
        success: false, 
        error: 'Gateway não configurado para esta empresa',
  ...
  try {
    ...
    const paymentResponse = await fetch(`${baseUrl}/payments`, {
      ...
      body: JSON.stringify({
        ...
        value: amount,
        dueDate: dueDate,
        description: description || 'Pagamento - CRM Imobiliário',
        externalReference: `crm-${Date.now()}`
      })
    })
  ...
  try {
    ...
    const response = await fetch('https://api.pjbank.com.br/recebimentos', {
      ...
      body: JSON.stringify({
        ...
        logo_url: '',
        texto: description || 'Pagamento - CRM Imobiliário',
        grupo: 'CRM_PAYMENT'
      })
    })
